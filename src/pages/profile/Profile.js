  ...
  return (
    ...
    <div className="profile">
    	<motion.form id="profileForm" initial={{opacity: 0, y: '5rem'}} exit={{opacity: 0, y: '5rem'}} animate={{opacity: 1, y: 0}} layout onSubmit={updatePro? handleSubmit : !validatePassword? validateCredentialsUpdate : enableUpdate}>
			<h2>{updatePro? "Update Profile" : "Profile"}</h2>
			...
			{!checkDelete && <button disabled={loading}>{updatePro ? "Confirm" : !validatePassword? "Update Profile" : "Continue"}</button>}
			{!checkUpdate && <button disabled={loading} onClick={!validatePassword? validateCredentialsDelete : deleteProfile}>{!validatePassword? "Delete Profile" : "Continue"}</button>}
			{(updatePro || checkDelete) && <button disabled={loading} onClick={cancelProfileChange}>Cancel</button>}
            <p>
